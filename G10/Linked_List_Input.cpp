...
node* ReverseLL_Recursive(node* head) {
	...
	//Main Recursion ko Bolta hu ki aage ka kaam tu karke mujhe dede:
	node* next_head = ReverseLL_Recursive(head->next);

	//Ek bar main kaam karta hu.
	node* current = head;
...
int main() {
	...
	node* head2 = ReverseLL_Recursive(head);
	Print(head2);
}
